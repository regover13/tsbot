...
def lese_config(skript_ordner: str) -> dict:
    config = {"ANTHROPIC_API_KEY": None, "CLAUDE_MODEL": "claude-sonnet-4-5-20250929"}
...
[
  ...
  {"name": "Anderer Name", "frs": "FRS12"}
    ...
    # Ähnliche Namen (Levenshtein-artig) → behalte häufigeren
    ...
    for eintrag in alle_roh:
        ...
        if frs:
            if any(e["frs"] == frs for e in eindeutig):
                # behalte längeren Namen
                for e in eindeutig:
                    if e["frs"] == frs and len(name) > len(e["name"]):
                        e["name"] = name
                continue
        # Duplikat per Namensähnlichkeit (>85%)
...
def ki_zuordnung(volltext: str, segmente: list, agenda: list, api_key: str, modell: str) -> list:
    try:
        import anthropic
    except ImportError:
        print("HINWEIS: anthropic-Paket nicht installiert.")
...
def erstelle_protokoll(transkript_pfad: str, thema: str,
    ...
    # Alle PNGs im Skript-Ordner finden
    pngs = finde_alle_pngs(skript_ordner)

    # Teilnehmer per Vision extrahieren
    teilnehmer_liste = []
    if pngs and hat_api:
    ...
    if agenda and hat_api:
        ki_punkte = ki_zuordnung(volltext, segmente, agenda, api_key, modell)
        if ki_punkte:
    ...
    # TOC-Feld einfügen (wird beim Öffnen in Word aktualisiert)
